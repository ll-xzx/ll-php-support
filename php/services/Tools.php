    ...
    protected function getPropertyArguments(ReflectionProperty $property)
    {
        ...
        if (!$docParseResult['var']['type']) {
            ...
            if ($data['wasFound']) {
                ...
                $className = (!empty($className) && substr($className, 0, 1) != '\\') ? '\\' . $className : $className;
                $docParseResult['var']['type'] = $className;
                $docParseResult['var']['description'] = $data['args']['descriptions']['short'];
                $docParseResult['descriptions'] = $data['args']['descriptions'];
                $docParseResult['deprecated'] = $data['args']['deprecated'];
            }
        }
        return array(
